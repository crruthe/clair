...
func (pgSQL *pgSQL) FindLayer(name string, withFeatures, withVulnerabilities bool) (database.Layer, error) {
	...
	t := time.Now()
	err := pgSQL.QueryRow(searchLayer, name).Scan(&layer.ID, &layer.Name, &layer.EngineVersion, &layer.ImageRef, &parentID, &parentName, &namespaceID, &namespaceName)
...
func (pgSQL *pgSQL) InsertLayer(layer database.Layer) error {
	...
	if layer.ID == 0 {
		// Insert a new layer.
		err = tx.QueryRow(insertLayer, layer.Name, layer.EngineVersion, layer.ImageRef, parentID, namespaceID).
			Scan(&layer.ID)
		if err != nil {
			tx.Rollback()
